...
import static org.xiaoxian.AxT.atbind.BindQQ.bindQQAndGroup;
import static org.xiaoxian.AxT.atbind.GetBindQQ.getBindGroup;
import static org.xiaoxian.AxT.atbind.WhiteListGroup.addWhiteListGroup;
import static org.xiaoxian.AxT.atbind.unBindQQ.*;

public class AxTGroupMsgListener implements Consumer<GroupMessageEvent> {
    @Override
    public void accept(GroupMessageEvent event) {
        ...
        if (event.getGroup().getId() == 198921528 || event.getGroup().getId() == 832275338 || event.getGroup().getId() == 660408793 || event.getGroup().getId() == 434756847){
            ...
            if (content.startsWith("/atbind rmall ")) {
                if (event.getSender().getId() == Config.getAdminQQ()) {
