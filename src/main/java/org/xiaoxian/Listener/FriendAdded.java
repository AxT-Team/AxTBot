...
public class FriendAdded implements Consumer<FriendAddEvent> {
    ...
    public void accept(FriendAddEvent event) {
        if (event.getBot().getGroup(832275338).getMembers().get(event.getFriend().getId()) == null && event.getBot().getGroup(660408793).getMembers().get(event.getFriend().getId()) == null) {
